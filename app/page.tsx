...
function SectionHeading({ eyebrow, title, text, align = "center" }: { eyebrow: string; title: string; text?: string; align?: "center" | "left" }) {
  const item = { hidden: { opacity: 0, y: 24, filter: "blur(8px)" }, visible: { opacity: 1, y: 0, filter: "blur(0px)", transition: { duration: .9, ease } } };
...
function CinematicImage({ src, alt, sizes, position = "center", priority = false }: { src: string; alt: string; sizes: string; position?: string; priority?: boolean }) {
  ...
  return <motion.div ref={target} className="cinematicImage" style={{ y: reducedMotion ? 0 : y, scale: reducedMotion ? 1 : scale }}>
...
function RingsTransition() {
  return <motion.section className="ringsTransition" initial="hidden" whileInView="visible" viewport={{ once: true, amount: .55 }} variants={{ hidden: {}, visible: { transition: { staggerChildren: .16 } } }}><motion.p className="eyebrow" variants={{ hidden: { opacity: 0, y: 12 }, visible: { opacity: 1, y: 0 } }}>TWO HEARTS · ONE PROMISE</motion.p><motion.div className="ringsArtwork" variants={{ hidden: { opacity: 0, scale: .62, rotate: -14, filter: "blur(12px)" }, visible: { opacity: 1, scale: 1, rotate: 0, filter: "blur(0px)", transition: { duration: 1.35, ease } } }}><span className="ringsGlow" /><Image src="/images/wedding-rings-engraved-clean.png" alt="Two interlocking gold wedding rings engraved with Jeremy and Anuja" width={1375} height={1144} sizes="(max-width: 640px) 78vw, 360px" /></motion.div><motion.h2 variants={{ hidden: { opacity: 0, y: 18 }, visible: { opacity: 1, y: 0, transition: { duration: .9, ease } } }}>Forever Begins Here</motion.h2><motion.i className="ringsRule" aria-hidden="true" variants={{ hidden: { scaleX: 0 }, visible: { scaleX: 1, transition: { duration: 1, ease } } }} /></motion.section>;
}
...
function SaveTheDateFilm() {
  ...
  const fadeFilmAudioIn = useCallback(async () => { const sound = filmAudio.current; const picture = video.current; if (!sound || !picture) return; stopAudioFade(); sound.currentTime = picture.currentTime; sound.volume = 0; await sound.play(); audioFade.current = window.setInterval(() => { sound.volume = Math.min(1, sound.volume + .08); if (sound.volume >= 1) stopAudioFade(); }, 70); }, [stopAudioFade]);
  const enableSound = useCallback(() => { const sound = filmAudio.current; const picture = video.current; if (!sound || !picture) return; soundUnlocked.current = true; stopAudioFade(); window.dispatchEvent(new Event("wedify-video-play")); sound.muted = false; sound.currentTime = picture.currentTime; sound.volume = 1; void sound.play().then(() => setSoundRequired(false)).catch(() => setSoundRequired(true)); }, [stopAudioFade]);
  useEffect(() => { const picture = video.current; const sound = filmAudio.current; if (!picture || !sound) return; let active = false; const fadeOut = () => { stopAudioFade(); window.dispatchEvent(new Event("wedify-video-leave")); if (sound.paused) { picture.pause(); return; } audioFade.current = window.setInterval(() => { sound.volume = Math.max(0, sound.volume - .1); if (sound.volume <= 0) { stopAudioFade(); sound.pause(); picture.pause(); } }, 60); }; const observer = new IntersectionObserver(([entry]) => { const shouldPlay = entry.isIntersecting && entry.intersectionRatio >= .6; if (shouldPlay === active) return; active = shouldPlay; if (active) { window.dispatchEvent(new Event("wedify-video-play")); picture.muted = true; void picture.play(); if (soundUnlocked.current) { setSoundRequired(false); void fadeFilmAudioIn(); } else setSoundRequired(true); } else fadeOut(); }, { threshold: [.2, .6] }); const ended = () => { active = false; sound.pause(); window.dispatchEvent(new Event("wedify-video-leave")); }; picture.addEventListener("ended", ended); observer.observe(picture); return () => { stopAudioFade(); observer.disconnect(); picture.removeEventListener("ended", ended); }; }, [fadeFilmAudioIn, stopAudioFade]);
...
export default function Home() {
  const [entered, setEntered] = useState(false); const [playing, setPlaying] = useState(false); const audio = useRef<HTMLAudioElement>(null); const fade = useRef<number | null>(null);
  const stopFade = useCallback(() => { if (fade.current !== null) { window.clearInterval(fade.current); fade.current = null; } }, []);
  const startMusic = useCallback(async () => { const element = audio.current; if (!element) return false; stopFade(); let volume = element.paused ? .05 : element.volume; element.volume = volume; try { if (element.paused) await element.play(); setPlaying(true); fade.current = window.setInterval(() => { volume = Math.min(.5, volume + .04); element.volume = volume; if (volume >= .5) stopFade(); }, 90); return true; } catch { setPlaying(false); return false; } }, [stopFade]);
  const toggleMusic = useCallback(() => { const element = audio.current; if (!element) return; if (element.paused) void startMusic(); else { stopFade(); element.pause(); setPlaying(false); } }, [startMusic, stopFade]);
  useEffect(() => { const fadeOutMusic = () => { const element = audio.current; if (!element || element.paused) return; stopFade(); fade.current = window.setInterval(() => { element.volume = Math.max(0, element.volume - .06); if (element.volume <= 0) { stopFade(); element.pause(); setPlaying(false); } }, 70); }; const restoreMusic = () => { void startMusic(); }; window.addEventListener("wedify-video-play", fadeOutMusic); window.addEventListener("wedify-video-leave", restoreMusic); return () => { window.removeEventListener("wedify-video-play", fadeOutMusic); window.removeEventListener("wedify-video-leave", restoreMusic); }; }, [startMusic, stopFade]);
  useEffect(() => { if (!entered || playing) return; const unlock = () => { void startMusic(); }; window.addEventListener("pointerdown", unlock, { once: true }); return () => window.removeEventListener("pointerdown", unlock); }, [entered, playing, startMusic]);
  useEffect(() => () => stopFade(), [stopFade]);
  return <MotionConfig reducedMotion="user" transition={{ duration: .7, ease }}><main><audio ref={audio} src={d.music} loop preload="auto" playsInline onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} /><AnimatePresence mode="wait">{!entered && <Intro key="intro" onEnter={() => setEntered(true)} onMusic={startMusic} />}</AnimatePresence>{entered && <motion.div className="siteContent" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><ScrollProgress /><Navigation />{!playing && <motion.button className="musicNudge" onClick={() => void startMusic()} initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}><span>♪</span><i><b>Wedding music</b><small>Tap to play</small></i></motion.button>}<Hero /><CountdownSection /><CoupleSection /><StorySection /><RingsTransition /><EventsSection /><GallerySection /><WeddingCamera /><RSVPSection /><SaveTheDateFilm /><ContactSection /><Footer /><FloatingActions playing={playing} toggleMusic={toggleMusic} /></motion.div>}</main></MotionConfig>;
}
