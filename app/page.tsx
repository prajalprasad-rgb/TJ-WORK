...
export default function Home() {
  const [entered, setEntered] = useState(false); const [playing, setPlaying] = useState(false); const [videoActive, setVideoActive] = useState(false); const audio = useRef<HTMLAudioElement>(null); const fade = useRef<number | null>(null); const musicUnlocked = useRef(false); const videoActiveRef = useRef(false);
  ...
  useEffect(() => { const fadeOutMusic = () => { videoActiveRef.current = true; setVideoActive(true); const element = audio.current; if (!element || element.paused) return; stopFade(); fade.current = window.setInterval(() => { element.volume = Math.max(0, element.volume - .12); if (element.volume <= 0) { stopFade(); element.pause(); setPlaying(false); } }, 50); }; const restoreMusic = () => { videoActiveRef.current = false; setVideoActive(false); if (musicUnlocked.current) void startMusic(); }; window.addEventListener("wedify-video-play", fadeOutMusic); window.addEventListener("wedify-video-leave", restoreMusic); return () => { window.removeEventListener("wedify-video-play", fadeOutMusic); window.removeEventListener("wedify-video-leave", restoreMusic); }; }, [startMusic, stopFade]);
