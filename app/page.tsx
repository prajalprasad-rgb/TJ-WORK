...
function SaveTheDateFilm({ onEnterVideo, onLeaveVideo }: { onEnterVideo: () => void; onLeaveVideo: () => void }) {
  ...
  useEffect(() => { const picture = video.current; const sound = filmAudio.current; if (!picture || !sound) return; let active = false; const fadeOut = () => { filmActive.current = false; stopAudioFade(); onLeaveVideo(); if (sound.paused) { picture.pause(); return; } audioFade.current = window.setInterval(() => { sound.volume = Math.max(0, sound.volume - .1); if (sound.volume <= 0) { stopAudioFade(); sound.pause(); picture.pause(); } }, 60); }; const observer = new IntersectionObserver(([entry]) => { const shouldPlay = entry.isIntersecting && entry.intersectionRatio >= .18; if (shouldPlay === active) return; active = shouldPlay; filmActive.current = active; if (active) { onEnterVideo(); picture.muted = true; void picture.play(); if (soundUnlocked.current) { setSoundRequired(false); void fadeFilmAudioIn(); } else setSoundRequired(true); } else fadeOut(); }, { threshold: [.05, .18] }); const ended = () => { active = false; filmActive.current = false; sound.pause(); onLeaveVideo(); }; picture.addEventListener("ended", ended); observer.observe(picture); return () => { filmActive.current = false; stopAudioFade(); observer.disconnect(); picture.removeEventListener("ended", ended); }; }, [fadeFilmAudioIn, onEnterVideo, onLeaveVideo, stopAudioFade]);
