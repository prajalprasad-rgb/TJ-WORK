...
function SaveTheDateFilm() {
  ...
  const enableSound = useCallback(() => { const sound = filmAudio.current; const picture = video.current; if (!sound || !picture) return; soundUnlocked.current = true; window.dispatchEvent(new Event("wedify-video-play")); sound.muted = false; void fadeFilmAudioIn().then(() => setSoundRequired(false)).catch(() => setSoundRequired(true)); }, [fadeFilmAudioIn]);
...
export default function Home() {
  const [entered, setEntered] = useState(false); const [playing, setPlaying] = useState(false); const [videoActive, setVideoActive] = useState(false); const audio = useRef<HTMLAudioElement>(null); const fade = useRef<number | null>(null); const musicUnlocked = useRef(false);
  const stopFade = useCallback(() => { if (fade.current !== null) { window.clearInterval(fade.current); fade.current = null; } }, []);
  const startMusic = useCallback(async () => { const element = audio.current; if (!element) return false; stopFade(); let volume = element.paused ? .05 : element.volume; element.volume = volume; try { if (element.paused) await element.play(); musicUnlocked.current = true; setPlaying(true); fade.current = window.setInterval(() => { volume = Math.min(.5, volume + .04); element.volume = volume; if (volume >= .5) stopFade(); }, 90); return true; } catch { setPlaying(false); return false; } }, [stopFade]);
  ...
  useEffect(() => { const fadeOutMusic = () => { setVideoActive(true); const element = audio.current; if (!element || element.paused) return; stopFade(); fade.current = window.setInterval(() => { element.volume = Math.max(0, element.volume - .08); if (element.volume <= 0) { stopFade(); element.pause(); setPlaying(false); } }, 60); }; const restoreMusic = () => { setVideoActive(false); if (musicUnlocked.current) void startMusic(); }; window.addEventListener("wedify-video-play", fadeOutMusic); window.addEventListener("wedify-video-leave", restoreMusic); return () => { window.removeEventListener("wedify-video-play", fadeOutMusic); window.removeEventListener("wedify-video-leave", restoreMusic); }; }, [startMusic, stopFade]);
  useEffect(() => { if (!entered || playing || videoActive || musicUnlocked.current) return; const unlock = () => { void startMusic(); }; window.addEventListener("pointerdown", unlock, { once: true }); return () => window.removeEventListener("pointerdown", unlock); }, [entered, playing, videoActive, startMusic]);
